...
% Last Modified by GUIDE v2.5 25-Jun-2017 16:10:56
...
% display(vertices);
display('*************************handels.figure1******************');
display(handles.figure1);
...
% h.ActionPostCallback = @display_text_CreateFcn;
%set(h, 'ActionPreCallback', @rotate3d_pre_Callback);
% set(h, 'ActionPostCallback', @rotate3d_post_Callback);
set(h, 'Enable', 'on');
...
%     display(get(hObject, 'Min'));
%     display(get(hObject, 'Max'));
...
% --- Executes during object creation, after setting all properties.
% function display_text_CreateFcn(obj, evd, handles)
% % hObject    handle to display_text (see GCBO)
% % eventdata  reserved - to be defined in a future version of MATLAB
% % handles    empty - handles not created until after all CreateFcns called
% disp('A rotation is about to occur.');
% newview = round(evd.Axes.View);
% set(handles.display_text, 'String', sprintf('The new view is [%d %d].',newView));
% % set(handles.text_annotation_dir, 'String', anno_dir);
...
newView = round(evd.Axes.View);
disp(sprintf('The new view is [%d %d].',newView));
